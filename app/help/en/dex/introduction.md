...
direct exchange of digital goods traded in the Index ecosystem.
...
briefly below. Although the Index DEX comes with all of them, it is up to
...
* **Separation of Powers**: 
  ...
  issuing IOUs and for processing the order book. In Index, order matching
  is performed by the protocol, which is unaware of implications concerning the
  involved assets.
  
* **Global Unified Order Book**:
  Since Index is global, anybody with an internet access can use the DEX for
  trading. This brings the world's liquidity to a single order book for
  decentralized trading.
  
* **Trade Almost Anything**:
  The Index DEX is asset agnostic. Hence you can trade at **any** pair.
...
* **No Limits**:
  The Index protocol is unable to limit your trading experience.
  
* **Decentralized**:
  The DEX is decentralized across the globe. This not only means that there is
  no single point of failure, but it also implies that the Index exchange is
...
* **Fast**:
  In contrast to other decentralized networks, the Index DEX allows for
  real-time trading and is only limited by the speed of light and the size of
  the planet.
  
* **Provable Order Matching Algorithm**:
  What makes the Index DEX unique is the provable order matching algorithm.
...
* **Collateralized Smartcoins**:
  One of the biggest features of Index are its *smartcoins* such as bitUSD,
  ...
  backed by Index' company shares (BTS) being locked up as collateral and
